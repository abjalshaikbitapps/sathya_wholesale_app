...
export default function CreateOrUpdateTypeForm({ initialValues }: IProps) {
  ...
  console.log(initialValues,'png');
  const { categories, loading } = useCategoriesQuery({
    limit: 999,
    type: "satya-wholesale",
    language: locale,
  });
  var catfields:any;
  if(categories)
  {
    catfields=categories.filter(d=>d.parent_id==null);
  }
  console.log(catfields,'mumbai');
  return (
        ...
        <Card className="w-full sm:w-8/12 md:w-2/3">
          <div>
            {fields.map((item: any & { id: string }, index: number) => (
             ...
             {...register(`banners.${index}.category` as const)}>
            {catfields&&catfields.map((item:any) => (
