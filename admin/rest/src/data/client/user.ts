...
export const userClient = {
  ...
  register: (variables: RegisterInput) => {
    return HttpClient.post<any>(API_ENDPOINTS.REGISTER, variables);
  },
  verifyemail: (variables: VerifyEmail) => {
    return HttpClient.post<AuthResponse>(API_ENDPOINTS.VERIFY, variables);
  },
  registeradmin: (variables: RegisterInput) => {
    return HttpClient.post<AuthResponse>(API_ENDPOINTS.REGISTERADMIN, variables);
  },
  ...
  fetchCustomers: ({ ...params }: Partial<UserQueryOptions>) => {
    return HttpClient.get<UserPaginator>(API_ENDPOINTS.CUSTOMER_LIST, {
      searchJoin: 'and',
      ...params,
    });
  },
  fetchUserList: ({ ...params }: Partial<UserQueryOptions>) => {
    return HttpClient.get<UserPaginator>(API_ENDPOINTS.USER_LIST, {
      searchJoin: 'and',
      ...params,
    });
  },
