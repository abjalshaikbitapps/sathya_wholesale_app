...
export const useVerifyMutation = () => {
  ...
  const { t } = useTranslation();
  const router=useRouter();
  return useMutation(userClient.verifyemail, {
    
    onSuccess: (data) => {
      if(data.permissions&&data.token)
      {

        toast.success(t('Verified Succesfully'));
        router.push(Routes.dashboard);
...
export const useRegisterAdminMutation = () => {
  ...
  return useMutation(userClient.registeradmin, {
    onSuccess: () => {
      toast.success(t('common:successfully-register'));
    },
    // Always refetch after error or success:
    onSettled: () => {
      queryClient.invalidateQueries(API_ENDPOINTS.REGISTERADMIN);
    },
  });
};
...
export const useUpdateUserMutation = () => {
  ...
  return useMutation(userClient.update, {
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries(API_ENDPOINTS.CUSTOMER_LIST);
...
export const useCustomersQuery = (params: Partial<QueryOptionsType>) => {
  const { data, isLoading, error } = useQuery<UserPaginator, Error>(
    [API_ENDPOINTS.CUSTOMER_LIST, params],
...
export const useUsersListQuery = (params: Partial<QueryOptionsType>) => {
  const { data, isLoading, error } = useQuery<UserPaginator, Error>(
    [API_ENDPOINTS.USER_LIST, params],
    () => userClient.fetchUserList(params),
    {
      keepPreviousData: true,
    }
  );
