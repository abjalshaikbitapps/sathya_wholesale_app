...
class ProductController extends CoreController
{
    ...
    public function fetchProducts(Request $request)
    {
        ...
        return $this->repository->with(['type', 'shop','categories'])->where('language', $language)->whereNotIn('id', $unavailableProducts);
    }
                ...
                try {
                    ...
                    if (isset($type->id)) {
                        ...
                        $valuobj=[
                            ...
                            'statetax'          =>$product['statetax'],
                            'countrytax'        =>$product['countrytax'],
                            'itemshortdescription'=>$product['itemshortdescription'],
                            'itemno'=>$product['itemno']
    
                        ];
                        $valueproduct=$this->repository->create($valuobj);
    
                           $category=DB::table('categories')
                           ->where('name', 'LIKE', '%' . $product['subcategory'] . '%')
                           ->get();
                        //    return response()->json($category);
                           $catarray = [$category[0]->parent, $category[0]->id];
    
                           $valueproduct->categories()->attach($catarray);
                    }
                } catch (Exception $e) {
                    //
                    throw new MarvelException("wron",$e);
                }
            }
            return true;
        }
    ...
    public function variationProducts(Request $request)
    {
        $requestFile = $request->file();
        $user = $request->user();
        $shop_id = $request->shop_id;
        ...
        if (isset($shop_id)) {
            ...
            $products = $this->repository->csvToArray(storage_path() . '/app/public/' . $file);
            // $product = Product::findOrFail($request['itemno']);
            $product= DB::table('products')
            ->where('itemno', $request['itemno'])
            ->get();

            return response()->json($request['itemno']);
            foreach ($products as $key => $product) {
                if (!isset($product['type_id'])) {
                    throw new MarvelException("MARVEL_ERROR.WRONG_CSV");
                }
    ...
    public function popularProducts(Request $request)
    {
        ...
        $products_query = $this->repository->with(['type', 'categories','shop'])->orderBy('created_at','desc')->where('language', $language);
        // $product    = $this->findOneByFieldOrFail('slug', $slug);
        //     $categories = $product->categories->pluck('id');
        $categories= $request->catvariables;
        if($categories==[0]){
            return $products_query->with('type')->limit(60)->get();
        }
            return $products_query->whereHas('categories', function ($query) use ($categories) {
