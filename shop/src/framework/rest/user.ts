...
export function useLogin() {
  ...
  const { mutate, isLoading } = useMutation(client.users.login, {
    onSuccess: (data) => {
      ...
      if(data?.business_license==null&& data?.business_license_verify==null){
...
export function useRegister() {
  ...
  const { mutate, isLoading } = useMutation(client.users.register, {
    onSuccess: (data) => {
      ...
      if(data.message)
      {
        toast.success('Successfully Registered  Please Verify Email!');
        closeModal();
        return;
      }
     else
...
//   const [_, setAuthorized] = useAtom(authorizationAtom);
//   const { closeModal } = useModalAction();
//   let [formError, setFormError] = useState<Partial<VerifyEmail> | null>(
//     null
//   );
//   const router = useRouter();
...
export function useVerifyMutation() {
  ...
  const router = useRouter();
  return useMutation(client.users.verifyemail, {
    onSuccess: (data) => {
      if (data?.token && data?.permissions?.length ){
...
export function useThreeplQuery() {
  return useQuery([API_ENDPOINTS.THREEPL],client.threeplclient.analytics );
}